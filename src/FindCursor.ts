...
export class FindCursor<TSchema extends Document = Document> {
  ...
  constructor(
    ...
    collectionProxy: ICollectionProxy<TSchema>
  ) {
    this.filter = filter
  ...
  async toArray(): Promise<WithId<TSchema>[]> {
    return this.collectionProxy._executeFindWithOptions(
      this.filter,
      this.options
    )
  }
  ...
  async next(): Promise<WithId<TSchema> | null> {
    const results = await this.collectionProxy._executeFindWithOptions(
      this.filter,
